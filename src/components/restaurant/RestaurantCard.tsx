...
export default function RestaurantCard({
  ...
  const IMG_SIZE = 72;
  const STATUS_TEXT = restaurant.status === "open" ? "aberto" : "fechado";
  ...
  return (
    <article>
      <Link
        ...
        aria-label={`${restaurant.name} - ${STATUS_TEXT}`}
      >
          ...
          <div className="flex items-center gap-4xs">
            ...
            >
              {deliveryFeeStatus(restaurant.deliveryFee) === "free" ? (
                ...
                <AiqentregaIcon aria-hidden="true" />
              )}
            ...
            <div className="dot-separator ml-2xs" aria-hidden="true" />

            <div
              className="flex items-center gap-2xs"
              aria-label={`Avaliação: ${restaurant.rating}`}
            >
